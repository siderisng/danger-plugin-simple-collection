...
let coverageFilePath = getFileAbsolutePath(
  'reports/danger/coverage-current.json',
)
let coverageFilePathPrev = getFileAbsolutePath(
  'reports/danger/coverage-develop.json',
)
  ...
  constructor() {
    this.getFileCoverageStats = filePath => {
      ...
      if (fileCoverage) {
        ...
        return [
          this.getCoverageState(branches),
          this.getCoverageState(statements),
        ]
      }
      return [0, 0]
    }
    ...
    this.getCoverageForFile = filePath =>
      this.coverageByPath[getFileAbsolutePath(filePath)]
  }
...
export default function coverage(reportsPath = 'reports/danger') {
  ...
  coverageFilePathPrev = getFileAbsolutePath(
    reportsPath + '/coverage-develop.json',
  )
  ...
  if (
    coverageParser.coverageFileExists() &&
    coverageParserPrev.coverageFileExists(coverageFilePathPrev)
  ) {
    ...
    if (
      ...
      warn(
        'No coverage develop file available, you should have a coverage-develop.json file in your reports.',
      )
    } else if (
      !coverageParser.coverageFileExists() &&
      coverageParserPrev.coverageFileExists(coverageFilePathPrev)
